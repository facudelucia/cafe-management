...
@NamedQuery(name="Product.getProductById", query="select new com.example.cafe.wrapper.ProductWrapper(p.id,p.name,p.description,p.price,p.status,p.category.id,p.category.name) from Product p where p.id=:id")
